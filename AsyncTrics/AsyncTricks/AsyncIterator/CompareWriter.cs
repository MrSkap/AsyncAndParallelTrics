...
public class CompareWriter
{
    ...
    /// <summary>
    ///     Получить поток пар для сравнения и обработать их параллельно.
    /// </summary>
    /// <param name="pairsCount">Кол-во пар.</param>
    /// <param name="min">Минимальное значение.</param>
    /// <param name="max">Максимальное значение.</param>
    /// <param name="cancellationToken">Токен отмены.</param>
    public async Task GeneratePairsAndParallelCompareAsync(int pairsCount, int min, int max,
        CancellationToken cancellationToken)
    {
        var foos = _dataGenerator.GetFoos(pairsCount, min, max);
